...
function AllVideosPage() {
  ...
  const deletingIds = useSelector((state: RootState) => state.videos.deletingIds)
  ...
  const handleDeleteVideo = (videoId: string, videoTitle: string) => {
    const confirmed = window.confirm(
      `Alert: Are you sure you want to delete "${videoTitle}"?\n\nThis action cannot be undone.`,
    )
              ...
              {videosLoading
                ? Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className="border-b border-white/5">
                    <td className="px-6 py-5" colSpan={10}>
                    ...
                    <td className="px-6 py-5 align-top">
                      <div className="min-w-[320px]">
                        ...
                        <p className="mt-2 max-w-md text-sm leading-6 text-slate-400">
                          {truncateWords(video.description, 25)}
                        </p>
                      </div>
                    </td>
